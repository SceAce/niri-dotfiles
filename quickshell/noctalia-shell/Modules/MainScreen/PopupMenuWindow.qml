...
PanelWindow {
  ...
  property bool clickCatcherArmed: false
  ...
  function open() {
    ...
    clickCatcherArmed = false;
    clickCatcherArmTimer.restart();
  }
  ...
  MouseArea {
    anchors.fill: parent
    enabled: root.visible && root.clickCatcherArmed
    acceptedButtons: Qt.LeftButton | Qt.RightButton | Qt.MiddleButton
    onClicked: root.close()
  }

  // Avoid immediately closing a freshly opened popup from the same pointer event
  // that created it. This is especially important for tray/context menus running
  // through a top-layer click-catcher window on some compositors.
  Timer {
    id: clickCatcherArmTimer
    interval: 120
    repeat: false
    onTriggered: {
      if (root.visible) {
        root.clickCatcherArmed = true;
      }
    }
  }
