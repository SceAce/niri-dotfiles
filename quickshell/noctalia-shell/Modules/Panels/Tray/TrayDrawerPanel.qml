            ...
            MouseArea {
              ...
              onClicked: mouse => {
                           ...
                           if (panelContent.popupMenuWindow && panelContent.popupMenuWindow.visible) {
                             ...
                             if (modelData.hasMenu && modelData.display && trayIcon.Window && trayIcon.Window.window) {
                               const windowPos = trayIcon.mapToItem(null, 0, 0);
                               let menuX = windowPos.x + Math.round(trayIcon.width / 2);
                               let menuY = windowPos.y + Math.round(trayIcon.height / 2);
                               const barPosition = Settings.getBarPositionForScreen(root.screen?.name);

                               if (barPosition === "left") {
                                 menuX = windowPos.x + trayIcon.width;
                               } else if (barPosition === "right") {
                                 menuX = windowPos.x;
                               } else if (barPosition === "top") {
                                 menuY = windowPos.y + trayIcon.height;
                               } else if (barPosition === "bottom") {
                                 menuY = windowPos.y;
                               }

                               modelData.display(trayIcon.Window.window, menuX, menuY);
                             } else if (modelData.hasMenu && modelData.menu && panelContent.trayMenu && panelContent.trayMenu.item) {
                               const barPosition = Settings.getBarPositionForScreen(root.screen?.name);
